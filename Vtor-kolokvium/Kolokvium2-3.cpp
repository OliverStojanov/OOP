...
class Demonstrator : public Student, public Predavach{
    private:
        int brCasNed;
    public:
        Demonstrator(int ind,int *oce, int bo,char *i,Kurs *k,int bk, int brCas) : Student(ind,oce,bo), Predavach(i,k,bk){
            brCasNed=brCas;
        }
        int getBodovi(){
            if(getBrojKursevi()==0){
                ...
                return Student::getBodovi()+(20*brCasNed)/getBrojKursevi();
            }
        }
        void pecati(){
            Student::pecati();
